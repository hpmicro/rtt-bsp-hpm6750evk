...
rt_err_t eeprom_i2c_write(struct rt_i2c_bus_device *i2c_bus, rt_uint8_t addr, rt_uint8_t *data, rt_uint32_t len)
{
   ...
   rt_size_t result;
    ...
    result = rt_i2c_master_send(i2c_bus, EEPROM_I2C_ADDR, RT_I2C_WR, buf, len+1);
...
rt_err_t eeprom_i2c_read(struct rt_i2c_bus_device *i2c_bus, rt_uint8_t addr, rt_uint8_t *buf, rt_uint32_t len)
{
    ...
    result = rt_i2c_master_send(i2c_bus, EEPROM_I2C_ADDR, RT_I2C_WR, &addr, 1);
    if (result != 1) {
        return RT_ERROR;
    }

    result = rt_i2c_master_recv(i2c_bus, EEPROM_I2C_ADDR, RT_I2C_RD, buf, len);
    if (result != len) {
        return RT_ERROR;
    }
...
static int eeprom_i2c_sample(int argc, char *argv[])
{
    ...
    eeprom_i2c_write(i2c_dev, eeprom_reg_addr, wr_data, data_count);
    rt_thread_mdelay(10);
    eeprom_i2c_read(i2c_dev, eeprom_reg_addr, rd_data, data_count);

    rt_kprintf("eeprom write %d bytes from address 0x%02x:\n", data_count, eeprom_reg_addr);
    for(rt_uint32_t i = 0; i < data_count; i++)
    {
        rt_kprintf("0x%02x ", wr_data[i]);
    }
    rt_kprintf("\n");
    rt_kprintf("eeprom read %d bytes from address 0x%02x:\n", data_count, eeprom_reg_addr);
    for(rt_uint32_t i = 0; i < data_count; i++)
    {
...
MSH_CMD_EXPORT(eeprom_i2c_sample, eeprom_i2c sample);
